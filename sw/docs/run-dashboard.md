...
1. **Mode** — `Offline` runs `engine_sim.OfflineSimulation` end to end.
   ...
   ITCH/OUCH sockets (see the note below).
...
3. **Run Simulation** — POSTs to `/simulate` or `/simulate/online` and renders
   the response:
...
> internally. You'll see a live-looking run that completes with an empty
> blotter and a flat PnL curve. This is a real gap in the engine/API, not a
> UI bug — see `sw/engine/simulation/src/online_simulation.cpp`.
